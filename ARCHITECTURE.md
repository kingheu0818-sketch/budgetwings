...
当前版本的 Scout 还不是“完全自主规划搜索来源与调用链路”的 agentic loop，而是一个“查询编排 + LLM 价格抽取”的 pipeline：
- 用预先设计好的 query 模板覆盖机票、火车和周末出行等场景
- 由程序控制搜索、抓取和去重流程
- 最后交给 LLM 做结构化价格抽取与 Deal 标准化

这样设计的原因是：
- **稳定性更高**：固定查询模板更容易控制结果分布，减少一次运行被单一来源主导
- **成本可预测**：工具调用次数和 LLM 调用轮数更容易估算，便于长期定时运行
- **便于测试**：pipeline 各阶段边界清晰，更适合做单元测试、回归测试和评估对比

**v2.1 计划**

后续版本会在此基础上引入真正的 agentic loop：允许 LLM 自主多轮调用 `web_search` / `web_fetch` 等工具，动态决定继续搜索、切换来源、补抓详情页，再汇总为结构化 Deal。下面这部分描述的是该方向下的理想形态，用于说明 BudgetWings 的长期目标。
